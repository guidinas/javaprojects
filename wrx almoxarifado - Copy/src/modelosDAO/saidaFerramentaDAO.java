...
public class saidaFerramentaDAO {
     ...
     *
     * @param m
     * @return true ion case it was able to 
     */
    public static boolean criaSaidaFerramentaCaixa(ArrayList<saidaFerramentas> m) {
          try{
            Connection con;
            con = conexao.getConnection();
            PreparedStatement stmt;
            for(saidaFerramentas k: m){
                stmt = con.prepareCall("INSERT INTO saidaFerramenta (codFerramenta , codFuncionario,stat) VALUES(?, ?, 2)");
                stmt.setInt(1, k.getCodFerramenta());
                stmt.setInt(2, k.getCodFuncionario());
                stmt.execute();
            }
